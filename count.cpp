...
int main(int argc, char ** argv) {
  ...
  // Added by me
  //int tmp_ind = find((*items)[1]->data));
  int tmp_ind = items->find("kyle");
  if (tmp_ind != -1){
    std::cout << "Yay \n";
  }
  LinkedList<std::string> * tmp = items;
  tmp_ind = tmp->find("kyle");
  if (tmp_ind != -1){
    std::cout << "Yay 2\n";
  }
  //LinkedList<std::string> tmp2 = items[1];
  //std::string tmp2 = items[1];
